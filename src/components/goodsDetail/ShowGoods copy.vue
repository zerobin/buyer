  ...
  <div class="wrapper">
    <div class="item-detail-show">
      ...
      <div class="item-detail-right">
        ...
        <div class="item-select-column" v-for="(objVal, key) of skuType">
          <div class="item-select-title">
            <p>{{ key }}</p>
          </div>
          <div style="width: 75%" class="item-select" v-if="key">
            <div v-for="(sku, index) in objVal.value" :key="index">
              <div
                class="item-select-intro skys-item"
                :class="[objVal.active === index ? 'active' : '']"
                v-if="objVal.unDisable && objVal.unDisable.includes(index)"
                @click="selectSku(sku, key, index)"
              >
                <p>{{ sku }}</p>
              </div>
              <div
  ...
  methods: {
    formatButtonText() {
      ...
      this.skuDetail.skus.forEach((skuItem, index) => {
        ...
        skuButtonList.push(skuDesc);
      });
      console.log(skuButtonList, "000000000000000000");
    },
    getPrice() {
    ...
    extractSpecList() {
      ...
      console.log(this.skuDetail.skus, "----ddd");
      this.skuDetail.skus &&
      ...
      let firstKey = Object.keys(this.skuType);
      if (firstKey.length) {
        let val = this.skuDetail.skus[0][firstKey[0]];
        let key = firstKey[0];
        let activedIndex = this.skuType[firstKey[0]].value.findIndex(
          (item) => item === val
        );
        console.log(
          this.skuDetail.skus[0][firstKey[0]],
          activedIndex,
          "=====ddd"
        );
        this.selectSku(val, key, activedIndex);
        // this.findOtherAttrIsDisable(
        //   firstKey[0],
        //   this.skuDetail.skus[0][firstKey[0]]
        // );
      }
    ...
    selectSku(value, key, actived) {
      // 选择规格
      console.log(value, key, "===");
      this.skuType[key].active = actived;
      Object.keys(this.skuType).forEach((item) => {
        let objKey = this.skuType[item];
        let actived = objKey.active;
        this.$set(this.selectedSpecs, item, objKey.value[actived]);
      });
      console.log(this.selectedSpecs);
      // if (this.matchingIndex !== -1) {
      //   this.skusImgSelect = true;
      // } else {
      //   this.skusImgSelect = false;
      // }

      this.findOtherAttrIsDisable(key, value);
      this.findMatchingSkuIndex();
    },
    // 找到不能点击的sku
    findOtherAttrIsDisable(key, value) {
      let filterKeySkus = this.skuDetail.skus.filter((sku) => {
        return sku[key] === value;
      });
      console.log(filterKeySkus, key, value, "00000ddd");
      let originalSkuType = JSON.parse(JSON.stringify(this.skuType));
      for (const i in originalSkuType) {
        originalSkuType[i].unDisable = [];
      }
      console.log(originalSkuType[key], "---");
      // originalSkuType[key].unDisable = [];
      originalSkuType[key].unDisable.push(
        originalSkuType[key].value.findIndex(
          (disableItem) => disableItem === value
        )
      );
      for (const i in originalSkuType) {
        if (i !== key) {
          filterKeySkus &&
            filterKeySkus.forEach((item, index) => {
              // if (!originalSkuType[i].unDisable) {
              //   originalSkuType[i].unDisable = [];
              // }

              originalSkuType[i].unDisable.push(
                originalSkuType[i].value.findIndex(
                  (disableItem) => disableItem === item[i]
                )
              );
              originalSkuType[i].unDisable = Array.from(
                new Set(originalSkuType[i].unDisable)
              );
            });
        }
      }
      this.skuType = JSON.parse(JSON.stringify(originalSkuType));
      console.log(filterKeySkus, "filterKeySkus");
      console.log(originalSkuType, "originalSkuType");
      // this.skuDetail.skus.findIndex((sku) => {
      //   return Object.keys(this.selectedSpecs).every((specName) => {
      //     return sku.specList.some(
      //       (spec) =>
      //         spec.specName === specName &&
      //         spec.specValue === this.selectedSpecs[specName]
      //     );
      //   });
      // });
    },
    findMatchingSkuIndex() {
      // 根据 selectedSpecs 中的规格信息，在 skus 中找到匹配的索引
      this.matchingIndex = this.skuDetail.skus.findIndex((sku) => {
        return Object.keys(this.selectedSpecs).every((specName) => {
          return sku.specList.some(
            (spec) =>
              spec.specName === specName &&
      ...
      // matchingIndex 即为匹配的 SKU 索引，可以在这里进行进一步操作
      console.log("Matching SKU Index:", this.matchingIndex);
    },
    addShoppingCartBtn() {
      // 判断用户是否登录
      if (!this.isLogin) {
